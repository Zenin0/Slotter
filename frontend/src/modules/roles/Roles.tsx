...
function Roles() {
    ...
    const handleCreate = async () => {
        ...
        try {
            await RestService.post("/api/v1/role", {
    ...
    const handleEdit = async () => {
        ...
        try {
            await RestService.put(`/api/v1/role/${editingRole.id}`, {
                ...editingRole, name: editName, description: editDescription,
            });

            setEditingRole(null);
            await fetchRoles();
        } catch {
            setError(t("failedUpdateRole"));
        } finally {
            setEditSaving(false);
        }
    };
    ...
    const handleToggleActive = async (role: Role) => {
        try {
            await RestService.put(`/api/v1/role/${role.id}`, {
    ...
    return (<div className="min-h-screen bg-gray-100 dark:bg-gray-950 p-8">
            <div className="max-w-4xl mx-auto">
                ...
                {/* Error */}
                {error && (<div
                        className="mb-4 flex items-center justify-between bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 text-red-600 dark:text-red-400 px-4 py-3 rounded-xl text-sm">
                        <span>⚠️ {error}</span>
                        <button
                ...
                {/* Roles list */}
                <div
                    className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 divide-y divide-gray-50 dark:divide-gray-800">

                    {roles.length === 0 ? (<p className="text-sm text-gray-400 dark:text-gray-500 p-6">
                            {t("noRoles")}
                        </p>) : roles.map((role) => (
                        <div key={role.id} className="flex items-center justify-between px-6 py-4">
                            ...
                            <div className="flex items-center gap-4">
                                <div
                                    className="h-9 w-9 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold text-sm">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span
                                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${role.isActive ? "bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400" : "bg-red-50 dark:bg-red-900/30 text-red-500 dark:text-red-400"}`}>
                                ...
                                <button
                                    ...
                                    className="text-xs px-3 py-1 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                                >
                                    {role.isActive ? t("deactivate") : t("activate")}
                                </button>
                            </div>
                        </div>))}
                </div>
            ...
            {showCreate && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div
                        className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-6 w-full max-w-md">
